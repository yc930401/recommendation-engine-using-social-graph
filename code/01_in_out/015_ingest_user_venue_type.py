...
# Rename venue_type that contains /
c.execute('select rid, venue_type from venues')
types = c.fetchall()
for rid, type in types:
    if '/' in type:
        c.execute('''update venues set venue_type = ? where rid = ?;''', (type.replace('/ ', ''), rid))
    elif '&' in type:
        c.execute('''update venues set venue_type = ? where rid = ?;''', (type.replace('& ', ''), rid))
    elif '-' in type:
        c.execute('''update venues set venue_type = ? where rid = ?;''', (type.replace('-', ' '), rid))
...
c.execute('select uid, venue_type, count(*) from venues inner join tips on venues.rid = tips.rid group by uid, venue_type;')
weights = c.fetchall()
